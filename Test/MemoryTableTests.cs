...
public class MemoryTableTests
{
    ...
    {
        var table = MemoryTable.Create<TestStructClean>();
        Dictionary<int, TestStructClean> dict = new();
        for (int i = 1; i < 100; i++)
        {
            var s = TestStructClean.Create(i);
            s = table.Insert(s);
            dict[i] = s;
        }
        for (int i = 1; i < 100; i++)
        {
            var result = table.FindStructs(new TestStructClean() { Text = i.ToString() });
            Assert.AreEqual(1, result.Count);
            Assert.AreEqual(dict[i], result.First());
        }
    }
