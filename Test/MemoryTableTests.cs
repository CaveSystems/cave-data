    ...
    public class MemoryTableTests
    {
        ...
        public void Default()
        {
            ...
            for (var i = 0; i < 1000; i++)
            {
                ...
                {
                    Content = "", DateTime = new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Local).AddHours(i), Name = "host" + (i % 10), Level = TestEnum.A,
                    Source = "this"
                });
            }
            ...
            Assert.AreEqual(10,
                test.Count(Search.None, ResultOption.Group(nameof(SmallTestStruct.Content)) + ResultOption.Group(nameof(SmallTestStruct.Name))));
            var rows = typed.GetStructs(Search.None,
                ResultOption.Group(nameof(SmallTestStruct.Name)) + ResultOption.SortDescending(nameof(SmallTestStruct.Name)));
            ...
            for (var i = 0; i < 1000; i++)
            {
                ...
                {
                    ID = i + 1, Content = "Updated" + i, DateTime = new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Local).AddHours(i % 100), Name = "this",
                    Level = TestEnum.B, Source = "this"
                };
                typed.Update(e);
            ...
            for (var i = 0; i < 1000; i++)
            {
                var e = new SmallTestStruct
                {
                    ID = i + 1, Content = "Replaced", DateTime = new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Local).AddHours(i), Name = "this",
                    Level = TestEnum.B, Source = "this"
                };
                typed.Update(e);
                Assert.AreEqual(e, typed.GetStruct(i + 1));
        ...
        public void OrderByWithLimit()
        {
            ...
            CollectionAssert.AreEqual(array.OrderBy(a => a.Integer).ToList(),
                typed.GetStructs(Search.None, ResultOption.SortAscending(nameof(SmallTestStruct.Integer))));
            CollectionAssert.AreEqual(array.OrderBy(a => a.DateTime).ToList(),
                typed.GetStructs(Search.None, ResultOption.SortAscending(nameof(SmallTestStruct.DateTime))));
            CollectionAssert.AreEqual(array.OrderByDescending(a => a.Integer).ToList(),
                typed.GetStructs(Search.None, ResultOption.SortDescending(nameof(SmallTestStruct.Integer))));
            CollectionAssert.AreEqual(array.OrderByDescending(a => a.DateTime).ToList(),
                typed.GetStructs(Search.None, ResultOption.SortDescending(nameof(SmallTestStruct.DateTime))));
            CollectionAssert.AreEqual(array.OrderBy(a => a.Integer).SubRange(0, 3),
                typed.GetStructs(Search.None, ResultOption.SortAscending(nameof(SmallTestStruct.Integer)) + ResultOption.Limit(3)));
            CollectionAssert.AreEqual(array.OrderByDescending(a => a.Integer).SubRange(0, 3),
                typed.GetStructs(Search.None, ResultOption.SortDescending(nameof(SmallTestStruct.Integer)) + ResultOption.Limit(3)));
            CollectionAssert.AreEqual(array.OrderBy(a => a.DateTime).SubRange(0, 3),
                typed.GetStructs(Search.None, ResultOption.SortAscending(nameof(SmallTestStruct.DateTime)) + ResultOption.Limit(3)));
            CollectionAssert.AreEqual(array.OrderByDescending(a => a.DateTime).SubRange(0, 3),
        ...
        public void OrderByWithLimitWithIndex()
        {
            ...
            CollectionAssert.AreEqual(array.OrderByDescending(a => a.Date),
                typed.GetStructs(Search.None, ResultOption.SortDescending(nameof(TestStructClean.Date))));
            CollectionAssert.AreEqual(array.OrderBy(a => a.I).SubRange(0, 3),
                typed.GetStructs(Search.None, ResultOption.SortAscending(nameof(TestStructClean.I)) + ResultOption.Limit(3)));
            CollectionAssert.AreEqual(array.OrderByDescending(a => a.I).SubRange(0, 3),
                typed.GetStructs(Search.None, ResultOption.SortDescending(nameof(TestStructClean.I)) + ResultOption.Limit(3)));
            CollectionAssert.AreEqual(array.OrderBy(a => a.Date).SubRange(0, 3),
                typed.GetStructs(Search.None, ResultOption.SortAscending(nameof(TestStructClean.Date)) + ResultOption.Limit(3)));
            CollectionAssert.AreEqual(array.OrderByDescending(a => a.Date).SubRange(0, 3),
