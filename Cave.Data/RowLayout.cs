...
public sealed class RowLayout : IEquatable<RowLayout>, IEnumerable<IFieldProperties>
{
    ...
    {
        if ((value is not null) && (value.GetType() != field.ValueType))
        {
            if (field.ValueType is null) throw new InvalidOperationException("Field.ValueType has to be set!");
        ...
        if (field.FieldInfo is null) throw new InvalidOperationException("Cannot set value, FieldInfo is null!");
    ...
    /// <summary>No or invalid row layout. This is used for new instances with lazy initialization.</summary>
    public static RowLayout None { get; } = new RowLayout();
    ...
    /// <returns>The <see cref="Cave.Data.FieldProperties"/> instance.</returns>
    ...
    public static RowLayout CreateTyped(Type type, string? nameOverride = null, IStorage? storage = null, params string[] excludedFields)
    {
        ...
        lock (LayoutCache)
        {
            ...
            if (!LayoutCache.TryGetValue(cacheName, out var result))
            {
                ...
                result = new(tableName, [.. properties], type);
    ...
    public static RowLayout Load(DataReader reader)
    {
        ...
        var tableName = reader.ReadPrefixedString() ?? $"Table{Base32.Safe.Encode(RNG.UInt32)}";
    ...
    public string GetDisplayString(int fieldIndex, object? value, CultureInfo? culture = null)
    {
        ...
        var result = field.DisplayFormat switch
        {
            "FormatTimeSpan" or "TimeSpan" => field.DataType switch
            {
                DataType.TimeSpan => value is null ? string.Empty : ((TimeSpan)value).FormatTime(),
            ...
            _ => value is null ? string.Empty : field.DataType switch
            {
                ...
                DataType.Binary => Base64.NoPadding.Encode(value as byte[] ?? throw new InvalidDataException($"Cannot convert {value} to byte[]")),
    ...
    public int GetFieldIndex(string fieldName, StringComparison comparison, bool throwException)
    {
        ...
        field = fieldProperties.SingleOrDefault(f => true == f.AlternativeNames?.Any(n => n.Equals(fieldName, comparison)));
        if (field != null)
        {
            return field.Index;
        }
    ...
    public RowLayout GetMatching(RowLayout baseTableLayout, TableFlags flags)
    {
        ...
        return new(baseTableLayout.Name, [.. result.OrderBy(i => i.Index)], RowType);
    }
    ...
    /// <summary>Retrieves a string for the specified value. The string may be parsed back to a value using <see cref="ParseValue(int, string, string, IFormatProvider)"/>.</summary>
    /// <param name="fieldIndex">The field number.</param>
    /// <param name="value">The value.</param>
    ...
    public object? GetValue(int index, object item)
    {
        ...
        var field = fieldProperties[index];
        if (field.FieldInfo is null) throw new InvalidOperationException("Cannot get value, FieldInfo is null!");
        return field.FieldInfo.GetValue(item);
    }
        ...
        where TStruct : struct
    {
        ...
        for (var i = 0; i < FieldCount; i++)
        {
            ...
            if (field.FieldInfo is null) throw new InvalidOperationException("Cannot get values. FieldInfo is null!");
            var value = field.FieldInfo.GetValue(item);
    ...
    public RowLayout Remove(string fieldName)
    {
        ...
        return new(Name, [.. fieldProperties], RowType);
    }
