...
public class DatabaseInterfaceGenerator
{
    ...
    public string Generate()
    {
        ...
        {
            result.AppendLine();
            var typeString = table.Layout.SingleIdentifier == null ? table.ClassName : $"{table.Layout.SingleIdentifier.DotNetTypeName}, {table.ClassName}";
            result.AppendLine($"\t\t/// <summary>Gets a new <see cref=\"ITable{{{typeString}}}\"/> instance for accessing the <c>{table.TableName}</c> table.</summary>");
            result.AppendLine($"\t\tpublic static ITable<{typeString}> {table.GetterName} => new Table<{table.ClassName}>(GetTable(\"{table.TableName}\"));");
        }
