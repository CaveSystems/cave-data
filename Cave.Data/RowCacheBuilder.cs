...
/// <summary>
/// Provides a class caching all results fetched from another table during usage. This class can be used to replace any other <see cref="ITable"/> instance to
/// build up a row cache on the fly during normal usage.
/// </summary>
public class RowCacheBuilder : ITable
{
    ...
    public Row GetRow(Search? search = null, ResultOption? resultOption = null)
    {
        ...
        if (resultOption is null || resultOption.Mode != ResultOptionMode.Group)
        {
            Cache.Replace(row);
        }
        return row;
    ...
    /// <inheritdoc/>
    public IList<TValue> GetValues<TValue>(string fieldName, Search? search = null, ResultOption? resultOption = null) where TValue : IComparable<TValue> => Table.GetValues<TValue>(fieldName, search, resultOption);
