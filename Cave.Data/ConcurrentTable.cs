...
public class ConcurrentTable : ITable
{
        ...
        lock (writeLock)
        {
            ...
            while (readLock > 0)
            {
                if (wait > 0)
                {
                    ...
                    while ((readLock > 0) && (wait > 0 && watch!.ElapsedMilliSeconds < wait))
                    {
                        Monitor.Exit(this);
                        Thread.Sleep(1);
                        Monitor.Enter(this);
    ...
    /// <inheritdoc/>
    public Row GetRow(Search? search = null, ResultOption? resultOption = null) => ReadLockedFunc(() => BaseTable.GetRow(search, resultOption)) ?? throw new InvalidOperationException("GetRow returned no Row!");

    /// <inheritdoc/>
    public Row GetRowAt(int index) => ReadLockedFunc(() => BaseTable.GetRowAt(index)) ?? throw new InvalidOperationException("GetRowAt returned no Row!");
    ...
    /// <inheritdoc/>
    public IList<TValue> GetValues<TValue>(string fieldName, Search? search = null, ResultOption? resultOption = default)
        where TValue : IComparable<TValue>
        => ReadLockedFunc(() => BaseTable.GetValues<TValue>(fieldName, search, resultOption)) ?? [];

    /// <inheritdoc/>
    public Row Insert(Row row) => WriteLocked(() => BaseTable.Insert(row)) ?? throw new InvalidOperationException("Insert returned no Row!");
