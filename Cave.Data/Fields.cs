...
#error No code defined for the current framework or NETXX version define missing!
        ...
        if (fieldType.IsEnum)
        {
            return Enum.Parse(fieldType, value.ToString() ?? string.Empty, true);
        }
        ...
        string? content = null;
        {
            ...
            else
            {
                ...
                var method = value.GetType().GetMethod("ToString", BindingFlags.Public | BindingFlags.Instance, null, [typeof(IFormatProvider)], null);
                if (method != null)
                {
                    try
                    {
                        content = (string?)method.Invoke(value, new object[] { provider });
                    }
        ...
        {
            if (content is null) return DateTime.MinValue;
            ...
            if (DateTimeParser.TryParseDateTime(content, out var dt))
            {
                return dt;
            }
        }
        ...
        {
            if (content is null) return TimeSpan.Zero;
            try
            {
                if (content.Contains(':'))
                {
...
#else
                    return TimeSpan.Parse(content, provider);
#endif
                }

                if (content.EndsWith("ms", StringComparison.Ordinal))
                {
                    return new TimeSpan((long)Math.Round(double.Parse(content.SubstringEnd(2), provider) * TimeSpan.TicksPerMillisecond));
                }

                return content.EndsWith('s')
                    ? new TimeSpan((long)Math.Round(double.Parse(content.SubstringEnd(1), provider) * TimeSpan.TicksPerSecond))
                    : (object)new TimeSpan(long.Parse(content, provider));
            }
            catch (Exception ex)
            {
                throw new InvalidDataException($"Value '{content}' is not a valid TimeSpan!", ex);
            }
        }
        ...
        // parse from string
        {
            ...
            var method = fieldType.GetMethod("Parse", BindingFlags.Public | BindingFlags.Static, null, [typeof(string), typeof(IFormatProvider)], null);
            if (method != null)
            {
                try
                {
                    return method.Invoke(null, new object[] { content, provider });
                }
                catch (TargetInvocationException ex)
                {
                    if (ex.InnerException is not null) errors.Add(ex.InnerException);
                }
            }

            method = fieldType.GetMethod("Parse", BindingFlags.Public | BindingFlags.Static, null, [typeof(string)], null);
            if (method != null)
            {
                try
                {
    ...
    public static string? GetDescription(object value)
    {
        ...
        if (type.IsEnum)
        {
            ...
            var name = Enum.GetName(value.GetType(), value) ?? throw new InvalidOperationException("Could not get name of enum!");
            var field = type.GetField(name) ?? throw new InvalidOperationException($"Could not get field of enum name {name}!");
