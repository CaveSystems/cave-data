...
public abstract class Table : ITable
{
    ...
    /// <inheritdoc/>
    public abstract long Count(Search? search = null, ResultOption? resultOption = null);

    /// <inheritdoc/>
    public abstract void Delete(Row row);

    /// <inheritdoc/>
    public virtual void Delete(IEnumerable<Row> rows) => Commit(rows.Select(Transaction.Delete));
    ...
    /// <inheritdoc/>
    public abstract Row GetRow(Search? search = null, ResultOption? resultOption = null);
    ...
    /// <inheritdoc/>
    public abstract IList<Row> GetRows(Search? search = null, ResultOption? resultOption = null);
    ...
    /// <inheritdoc/>
    public virtual IList<TValue> GetValues<TValue>(string fieldName, Search? search = null, ResultOption? resultOption = null)
        where TValue : IComparable<TValue>
    {
        var index = Layout.GetFieldIndex(fieldName, true);
        var rows = GetRows(search, resultOption);
    ...
    /// <inheritdoc/>
    public virtual void Insert(IEnumerable<Row> rows) => Commit(rows.Select(Transaction.Insert));
        ...
        where TValue : struct, IComparable
    {
        ...
        var current = rows.First()[fieldNumber] as TValue? ?? default;
        foreach (var row in rows)
        {
            var value = row[fieldNumber] as TValue? ?? default;
    ...
    /// <inheritdoc/>
    public virtual void Replace(IEnumerable<Row> rows) => Commit(rows.Select(Transaction.Replace));
    ...
    public virtual double Sum(string fieldName, Search? search = null)
    {
        ...
        switch (field.DataType)
        {
            case DataType.TimeSpan:
                ...
                {
                    sum += (row[fieldNumber] as TimeSpan?)?.TotalSeconds ?? 0;
                }
    ...
    /// <inheritdoc/>
    public virtual void Update(IEnumerable<Row> rows) => Commit(rows.Select(Transaction.Updated));
